...
async function seed() {
  ...
  try {
    ...
    const insertedVehicleSpecs = await db.insert(vehicleSpecifications).values(vehicleSpecData).returning();
    ...
    const insertedSupportTickets = await db.insert(supportTickets).values(supportTicketData).returning();
